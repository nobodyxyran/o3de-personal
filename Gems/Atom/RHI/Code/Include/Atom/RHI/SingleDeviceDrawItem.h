...
namespace AZ::RHI
{
    ...
    using SingleDeviceDrawIndirect = SingleDeviceIndirectArguments;
    ...
    struct SingleDeviceDrawArguments
    {
        ...
        SingleDeviceDrawArguments(const SingleDeviceDrawIndirect& indirect)
    ...
    {
        SingleDeviceDrawItemProperties() = default;

        SingleDeviceDrawItemProperties(const SingleDeviceDrawItem* item, DrawItemSortKey sortKey = 0, DrawFilterMask filterMask = DrawFilterMaskDefaultValue)
            : m_item{item}
            , m_sortKey{sortKey}
