...
namespace AZ::RHI
{
    ...
    struct SingleDeviceIndirectArguments
    {
        SingleDeviceIndirectArguments() = default;

        SingleDeviceIndirectArguments(
        ...
        const SingleDeviceIndirectBufferView* m_indirectBufferView = nullptr;

        //! Optional count buffer that contains the number of indirect commands in the indirect buffer.
        const SingleDeviceBuffer* m_countBuffer = nullptr;
    };
}
