...
namespace AZ::RHI
{
        ...
        const MultiDeviceRayTracingBufferPools& rayTracingBufferPools)
    {
        ...
        IterateDevices(
            [this, &resultCode, &descriptor, &rayTracingBufferPools](int deviceIndex)
            {
                ...
                m_deviceObjects[deviceIndex] = Factory::Get().CreateRayTracingBlas();

                auto deviceDescriptor{ descriptor->GetDeviceRayTracingBlasDescriptor(deviceIndex) };

                resultCode = GetDeviceRayTracingBlas(deviceIndex)->CreateBuffers(
    ...
    bool MultiDeviceRayTracingBlas::IsValid() const
    {
        ...
        return IterateObjects<SingleDeviceRayTracingBlas>([]([[maybe_unused]] auto deviceIndex, auto deviceBlas)
        {
            if (!deviceBlas->IsValid())
            {
        ...
        const MultiDeviceRayTracingBufferPools& rayTracingBufferPools)
    {
        ...
        IterateDevices(
            ...
            {
                auto device = RHISystemInterface::Get()->GetDevice(deviceIndex);
                m_deviceObjects[deviceIndex] = Factory::Get().CreateRayTracingTlas();

                auto deviceDescriptor{ descriptor->GetDeviceRayTracingTlasDescriptor(deviceIndex) };

                resultCode = GetDeviceRayTracingTlas(deviceIndex)->CreateBuffers(
                    *device, &deviceDescriptor, *rayTracingBufferPools.GetDeviceRayTracingBufferPools(deviceIndex).get());
    ...
    const RHI::Ptr<RHI::MultiDeviceBuffer> MultiDeviceRayTracingTlas::GetTlasBuffer() const
    {
        ...
        ResultCode result = IterateObjects<SingleDeviceRayTracingTlas>([&tlasBuffer](auto deviceIndex, auto deviceTlas)
        {
            tlasBuffer->GetDeviceBuffer(deviceIndex) = deviceTlas->GetTlasBuffer();

            if (!tlasBuffer->GetDeviceBuffer(deviceIndex))
            {
                return ResultCode::Fail;
            }
    ...
    const RHI::Ptr<RHI::MultiDeviceBuffer> MultiDeviceRayTracingTlas::GetTlasInstancesBuffer() const
    {
        ...
        ResultCode result = IterateObjects<SingleDeviceRayTracingTlas>([&tlasInstancesBuffer](auto deviceIndex, auto deviceTlas)
        {
            tlasInstancesBuffer->GetDeviceBuffer(deviceIndex) = deviceTlas->GetTlasBuffer();

            if (!tlasInstancesBuffer->GetDeviceBuffer(deviceIndex))
            {
                return ResultCode::Fail;
            }
