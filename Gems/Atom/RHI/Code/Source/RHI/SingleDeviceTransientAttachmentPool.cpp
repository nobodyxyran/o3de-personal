...
namespace AZ::RHI
{
    bool SingleDeviceTransientAttachmentPool::NeedsTransientAttachmentPool(const TransientAttachmentPoolDescriptor& descriptor)
    {
    ...
    ResultCode SingleDeviceTransientAttachmentPool::Init(Device& device, const TransientAttachmentPoolDescriptor& descriptor)
    {
        if (Validation::IsEnabled())
        {
            if (IsInitialized())
            {
                AZ_Error("SingleDeviceTransientAttachmentPool", false, "SingleDeviceTransientAttachmentPool is already initialized!");
    ...
    void SingleDeviceTransientAttachmentPool::Shutdown()
    {
        if (IsInitialized())
        {
            ShutdownInternal();
    ...
    void SingleDeviceTransientAttachmentPool::Begin(const TransientAttachmentPoolCompileFlags compileFlags, const TransientAttachmentStatistics::MemoryUsage* memoryHint)
    {
    ...
    void SingleDeviceTransientAttachmentPool::BeginScope(Scope& scopeBase)
    {
    ...
    void SingleDeviceTransientAttachmentPool::EndScope()
    {
        m_currentScope = nullptr;
    }

    void SingleDeviceTransientAttachmentPool::End()
    {
        EndInternal();
    }

    const TransientAttachmentStatistics& SingleDeviceTransientAttachmentPool::GetStatistics() const
    {
        return m_statistics;
    }

    const TransientAttachmentPoolDescriptor& SingleDeviceTransientAttachmentPool::GetDescriptor() const
    {
        return m_descriptor;
    }

    TransientAttachmentPoolCompileFlags SingleDeviceTransientAttachmentPool::GetCompileFlags() const
    {
        return m_compileFlags;
    }

    void SingleDeviceTransientAttachmentPool::CollectHeapStats(AliasedResourceTypeFlags typeMask, AZStd::span<const TransientAttachmentStatistics::Heap> heapStats)
    {
    ...
    bool SingleDeviceTransientAttachmentPool::ValidateInitParameters([[maybe_unused]] const TransientAttachmentPoolDescriptor& descriptor) const
    {
